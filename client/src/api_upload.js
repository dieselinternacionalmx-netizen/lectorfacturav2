...
function parseInvoiceData(text, filename) {
    ...
    for (const knownAgent of KNOWN_AGENTS) {
        if (text.toUpperCase().includes(knownAgent.toUpperCase())) {
            agent = knownAgent;
            break;
        }
    }

    // Extract Client - Look for client name after "Datos del Cliente" section
    let client = '';
    const clientMatch = text.match(/Datos del Cliente:[\s\S]{0,200}Nombre:\s*([^\n]+)/i);
    if (clientMatch) {
        client = clientMatch[1].trim();
    } else {
        // Fallback: Look for any "Nombre:" field
        const genericClientMatch = text.match(/Nombre:\s*([^\n]+)/i);
        if (genericClientMatch) {
            client = genericClientMatch[1].trim();
        }
    }
